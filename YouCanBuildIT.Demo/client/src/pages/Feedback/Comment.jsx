...
const Comment = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log(comment, rating); // Process the comment and rating
  ...
  return (
    <div className={styles['comment-form-container']}>
      <form className={styles['comment-form']} onSubmit={handleSubmit}>
          ...
          {[...Array(5)].map((star, index) => {
            ...
            return (
              <button
                type="button"
                key={index}
                className={index <= rating ? styles['star-button'] : styles['star-button-blank']}
        ...
        <button type="submit" className={styles['btnSumit']}>Submit</button>
      </form>
    </div>
  );
};
